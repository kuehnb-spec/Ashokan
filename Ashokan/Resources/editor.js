...
Please report this to https://github.com/markedjs/marked.`, e) {
  ...
  var CONTAINER_TAGS = ["div", "section", "article", "aside", "header", "footer", "main", "nav", "figure"];
  var nodes = {
    doc: { content: "block+" },
    paragraph: {
      group: "block",
    ...
    code_block: {
      ...
      toDOM(node) {
        return ["pre", bagToDOM(node), ["code", node.attrs.codeAttrs || {}, 0]];
      }
    },
    ordered_list: {
      group: "block",
    ...
    task_checkbox: {
      ...
      parseDOM: [{
        ...
        getAttrs: (d2) => ({
          checked: d2.hasAttribute("checked") || d2.checked,
          attrs: bagFromDOM(d2, ["checked", "type"])
        })
      }],
      toDOM(node) {
  ...
  var tables2 = tableNodes({
    ...
    cellAttributes: {
      attrs: {
        default: {},
        getFromDOM(dom) {
          return bagFromDOM(dom, ["colspan", "rowspan", "data-colwidth"]);
        },
        setDOMAttr(value, domAttrs) {
          Object.assign(domAttrs, value || {});
        }
      }
    }
  });
  ...
  var INLINE_RAW_TAGS = /* @__PURE__ */ new Set(["wbr", "label", "output", "data", "bdi", "bdo", "ruby", "button", "input", "select"]);
  function isTaskCheckbox(el) {
    return el.tagName.toLowerCase() === "input" && el.getAttribute("type") === "checkbox";
  }
  var PHRASING_PARENTS = /* @__PURE__ */ new Set([
    "p",
    "h1",
    "h2",
    "h3",
  ...
  function serializeBodyHTML(doc3) {
    ...
    for (const table of wrap2.querySelectorAll("table")) {
      const rows = Array.from(table.querySelectorAll(":scope > tr"));
      if (!rows.length) continue;
      const headRows = [];
      while (rows.length && rows[0].children.length && Array.from(rows[0].children).every((c) => c.tagName === "TH")) {
  ...
  function imageNodeView(node, view2, getPos) {
    let current = node;
    const wrap2 = document.createElement("span");
    wrap2.className = "ashokan-image";
  ...
  function taskCheckboxView(node, view2, getPos) {
    let current = node;
  ...
  function insertImageFile(file, view2) {
  ...
  function changeAuthors(doc3) {
    const authors = /* @__PURE__ */ new Set();
    for (const change of collectChanges(doc3)) {
  ...
  function setPreviewMode(mode) {
    ...
    if (previewMode === "markup") {
      ...
      previewBanner.textContent = previewMode === "clean" ? "Previewing FINAL \u2014 markup hidden, still in the file \xB7 editing off" : "Previewing ORIGINAL \u2014 suggestions hidden, still in the file \xB7 editing off";
    ...
    view = new EditorView(place, {
      ...
      handleDOMEvents: {
        ...
        paste(view2, event) {
          for (const item of event.clipboardData?.items || []) {
            if (item.type.startsWith("image/")) {
              event.preventDefault();
              insertImageFile(item.getAsFile(), view2);
              return true;
            }
          }
          return false;
        ...
        drop(view2, event) {
          ...
          event.preventDefault();
          const drop = view2.posAtCoords({ left: event.clientX, top: event.clientY });
          if (drop) {
            view2.dispatch(view2.state.tr.setSelection(TextSelection.near(view2.state.doc.resolve(drop.pos))));
          }
          files.forEach((f) => insertImageFile(f, view2));
          return true;
        }
      },
  ...
  window.Ashokan = {
    ...
    italic() {
      run2(toggleMark(schema.marks.em));
    },
    underline() {
      run2(toggleMark(schema.marks.underline));
    },
    strike() {
      run2(toggleMark(schema.marks.strike));
    },
    inlineCode() {
      run2(toggleMark(schema.marks.code));
    },
    setHeading(level) {
      run2(setBlockType2(schema.nodes.heading, { level }));
    },
    setParagraph() {
      run2(setBlockType2(schema.nodes.paragraph));
    },
    toggleCodeBlock() {
      run2(setBlockType2(schema.nodes.code_block));
    },
    bulletList() {
      run2(wrapInList(schema.nodes.bullet_list));
    },
    orderedList() {
      run2(wrapInList(schema.nodes.ordered_list));
    },
    blockquote() {
      run2(wrapIn(schema.nodes.blockquote));
    },
    lift() {
      run2(lift2);
    },
    ...
    setLink(href) {
      if (!href) {
        run2(toggleMark(schema.marks.link));
        return;
      }
      run2(toggleMark(schema.marks.link, { attrs: { href } }));
    },
    insertImage(src, alt) {
      const attrs = { src };
    ...
    alignImage(mode) {
      ...
      let style = (attrs.style || "").split(";").map((s) => s.trim()).filter((s) => s && !/^(float|display|margin)\s*:/.test(s)).join("; ");
      const extra = {
        inline: "",
    ...
    insertTable(rows, cols) {
      ...
      const makeRow = (cellType) => table_row.create(null, Array.from({ length: cols }, () => cellType.createAndFill()));
    ...
    tableCommand(name) {
      ...
      if (commands[name]) run2(commands[name]);
    },
    undo() {
      run2(undo);
    },
    ...
    getReviewState() {
      ...
      const quote = (from2, to) => doc3.textBetween(from2, to, " ", " ");
      return {
        changes: collectChanges(doc3).map((c) => ({
          type: c.type,
          author: c.author,
          text: quote(c.from, c.to)
        })),
        comments: collectComments(doc3).map((c) => ({
          text: c.text,
          author: c.author,
          anchor: quote(c.from, c.to)
        }))
      };
    },
